...
export default async function transactions(req, res, id) {
  ...
  if (!id && req.method === 'POST') {
    const { fecha, monto, categoria_id, payment_method_id, descripcion } = req.body || {}
    if (!fecha || !monto) {
    ...
    const [row] = await sql`
      ...
      VALUES (${fecha}, ${monto}, ${categoria_id || null}, ${payment_method_id || null}, 'manual', ${descripcion || null})
      RETURNING *
    `
  ...
  if (id && req.method === 'PUT') {
    ...
    const [row] = await sql`
      UPDATE transactions SET fecha = ${fecha}, monto = ${monto},
